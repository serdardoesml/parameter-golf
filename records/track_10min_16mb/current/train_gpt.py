...
class LoopNorms(nn.Module):
	def __init__(self,dim):super().__init__();self.attn_norm=RMSNorm();self.mlp_norm=RMSNorm();self.attn_bias=nn.Parameter(torch.zeros(dim,dtype=torch.float32));self.mlp_bias=nn.Parameter(torch.zeros(dim,dtype=torch.float32))
class Block(nn.Module):
	...
	def forward(self,x,x0,norms=None):
		attn_norm=self.attn_norm if norms is None else norms.attn_norm;mlp_norm=self.mlp_norm if norms is None else norms.mlp_norm
		mix=self.resid_mix.to(dtype=x.dtype);x_in=mix[0][None,None,:]*x+mix[1][None,None,:]*x0;attn_in=attn_norm(x_in)*self.ln_scale_factor
		if norms is not None:attn_in=attn_in+norms.attn_bias.to(dtype=x_in.dtype)[None,None,:]
		attn_out=self.attn(attn_in)
		if self.parallel:
			mlp_in=mlp_norm(x_in)*self.ln_scale_factor
			if norms is not None:mlp_in=mlp_in+norms.mlp_bias.to(dtype=x_in.dtype)[None,None,:]
			mlp_out=self.mlp(mlp_in);x_out=x_in+self.attn_scale.to(dtype=x_in.dtype)[None,None,:]*attn_out+self.mlp_scale.to(dtype=x_in.dtype)[None,None,:]*mlp_out
		else:
			x_out=x_in+self.attn_scale.to(dtype=x_in.dtype)[None,None,:]*attn_out;mlp_in=mlp_norm(x_out)*self.ln_scale_factor
			if norms is not None:mlp_in=mlp_in+norms.mlp_bias.to(dtype=x_out.dtype)[None,None,:]
			x_out=x_out+self.mlp_scale.to(dtype=x_out.dtype)[None,None,:]*self.mlp(mlp_in)
		return x_out
class GPT(nn.Module):
	def __init__(self,h):
		super().__init__()
		...
		self.looping_active=False;self.loop_start=h.loop_start;self.loop_end=h.loop_end
		if h.num_loops>0:
			...
			self.loop_norms=nn.ModuleList([LoopNorms(h.model_dim)for i in all_indices if h.loop_start<=i<=h.loop_end])
		else:self.encoder_indices=list(range(self.num_encoder_layers));self.decoder_indices=list(range(self.num_encoder_layers,h.num_layers));self.loop_norms=nn.ModuleList()
	...
	def forward_logits(self,input_ids):
		...
		loop_norm_pos=0
		if len(self.loop_norms)>0 and not self.looping_active:
			dummy=sum((n.attn_bias.sum()+n.mlp_bias.sum())for n in self.loop_norms)*0.;x=x+dummy.to(dtype=x.dtype)
		for i in enc_iter:
			norms=None
			if self.looping_active and self.loop_start<=i<=self.loop_end:norms=self.loop_norms[loop_norm_pos];loop_norm_pos+=1
			x=self.blocks[i](x,x0,norms);skips.append(x)
...
class Optimizers:
	def __init__(self,h,base_model):
		block_named_params=list(base_model.blocks.named_parameters())+[(f"loop_norms.{name}",p)for(name,p)in base_model.loop_norms.named_parameters()];matrix_params=[p for(name,p)in block_named_params if p.ndim==2 and not any(pattern in name for pattern in CONTROL_TENSOR_NAME_PATTERNS)];scalar_params=[p for(name,p)in block_named_params if p.ndim<2 or any(pattern in name for pattern in CONTROL_TENSOR_NAME_PATTERNS)]
