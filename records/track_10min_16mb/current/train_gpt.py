...
class Block(nn.Module):
	def __init__(self,dim,mlp_mult,layer_idx=0,ln_scale=False,use_mlp=True):super().__init__();self.attn_norm=RMSNorm();self.attn_norm_bias=nn.Parameter(torch.zeros(dim,dtype=torch.float32));self.mlp_norm=RMSNorm()if use_mlp else None;self.mlp=MLP(dim,mlp_mult)if use_mlp else None;self.attn_scale=nn.Parameter(torch.ones(dim,dtype=torch.float32));self.mlp_scale=nn.Parameter(torch.ones(dim,dtype=torch.float32))if use_mlp else None;self.resid_mix=nn.Parameter(torch.stack((torch.ones(dim),torch.zeros(dim))).float());self.ln_scale_factor=1./math.sqrt(layer_idx+1)if ln_scale else 1.;self.parallel=False
	def forward(self,x,x0,attn):
...
class GPT(nn.Module):
	def __init__(self,h):
		...
		self.shared_attn=CausalSelfAttention(h.model_dim,h.num_heads,h.num_kv_heads,h.rope_base,h.qk_gain_init,h.train_seq_len)
		...
		if h.rope_dims>0:
			...
			self.shared_attn.rope_dims=h.rope_dims;self.shared_attn.rotary=Rotary(head_dim,base=h.rope_base,train_seq_len=h.train_seq_len,rope_dims=h.rope_dims)
		...
		if h.xsa_last_n>0:self.shared_attn.use_xsa=True
	...
	def forward_logits(self,input_ids):
		...
		for i in enc_iter:x=self.blocks[i](x,x0,self.shared_attn);skips.append(x)
		for(skip_idx,i)in enumerate(dec_iter):
			...
			x=self.blocks[i](x,x0,self.shared_attn)
		x=self.final_norm(x)
...
def classify_param(name):
	...
	if'.attn.'in name or'shared_attn'in name or'.proj.'in name and'.mlp.'not in name:return'attn'
	return'other'
@torch.compile
...
class Optimizers:
	def __init__(self,h,base_model):
		block_named_params=list(base_model.blocks.named_parameters())+[(f"shared_attn.{name}",p)for(name,p)in base_model.shared_attn.named_parameters()];matrix_params=[p for(name,p)in block_named_params if p.ndim==2 and not any(pattern in name for pattern in CONTROL_TENSOR_NAME_PATTERNS)];scalar_params=[p for(name,p)in block_named_params if p.ndim<2 or any(pattern in name for pattern in CONTROL_TENSOR_NAME_PATTERNS)]
