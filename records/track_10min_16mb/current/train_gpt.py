...
class GPT(nn.Module):
	def __init__(self,h):
		...
		self.num_encoder_layers=h.num_layers//2;self.num_decoder_layers=h.num_layers-self.num_encoder_layers;self.blocks=nn.ModuleList([Block(h.model_dim,h.num_heads,h.num_kv_heads,h.mlp_mult,h.rope_base,h.qk_gain_init,h.train_seq_len,layer_idx=i,ln_scale=h.ln_scale,use_attn2=h.loop_start<=i<=h.loop_end,use_mlp=i!=0)for i in range(h.num_layers)])
		if h.rope_dims>0:
