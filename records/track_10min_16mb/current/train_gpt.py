...
class Block(nn.Module):
	def __init__(self,dim,num_heads,num_kv_heads,mlp_mult,rope_base,qk_gain_init,train_seq_len,layer_idx=0,ln_scale=False,use_attn2=False):super().__init__();self.attn_norm=RMSNorm();self.mlp_norm=RMSNorm();self.attn=CausalSelfAttention(dim,num_heads,num_kv_heads,rope_base,qk_gain_init,train_seq_len);self.attn2_norm=RMSNorm()if use_attn2 else None;self.attn2=CausalSelfAttention(dim,num_heads,num_kv_heads,rope_base,qk_gain_init,train_seq_len)if use_attn2 else None;self.mlp=MLP(dim,mlp_mult);self.attn_scale=nn.Parameter(torch.ones(dim,dtype=torch.float32));self.attn2_scale=nn.Parameter(torch.ones(dim,dtype=torch.float32))if use_attn2 else None;self.mlp_scale=nn.Parameter(torch.ones(dim,dtype=torch.float32));self.resid_mix=nn.Parameter(torch.stack((torch.ones(dim),torch.zeros(dim))).float());self.ln_scale_factor=1./math.sqrt(layer_idx+1)if ln_scale else 1.;self.parallel=False
	def forward(self,x,x0):
		...
		if self.parallel:
			...
			x_out=x_out+self.mlp_scale.to(dtype=x_in.dtype)[None,None,:]*self.mlp(self.mlp_norm(x_in)*self.ln_scale_factor)
		else:
			x_out=x_in+self.attn_scale.to(dtype=x_in.dtype)[None,None,:]*attn_out
...
class GPT(nn.Module):
	def __init__(self,h):
		...
		self.num_encoder_layers=h.num_layers//2;self.num_decoder_layers=h.num_layers-self.num_encoder_layers;self.blocks=nn.ModuleList([Block(h.model_dim,h.num_heads,h.num_kv_heads,h.mlp_mult,h.rope_base,h.qk_gain_init,h.train_seq_len,layer_idx=i,ln_scale=h.ln_scale,use_attn2=h.loop_start<=i<=h.loop_end)for i in range(h.num_layers)])
		if h.rope_dims>0:
