...
class Block(nn.Module):
	def __init__(self,dim,mlp_mult,layer_idx=0,ln_scale=False,use_mlp=True):super().__init__();self.attn_norm=RMSNorm();self.attn2_norm=RMSNorm();self.attn_norm_bias=nn.Parameter(torch.zeros(dim,dtype=torch.float32));self.attn2_norm_bias=nn.Parameter(torch.zeros(dim,dtype=torch.float32));self.mlp_norm=RMSNorm()if use_mlp else None;self.mlp=MLP(dim,mlp_mult)if use_mlp else None;self.attn_scale=nn.Parameter(torch.ones(dim,dtype=torch.float32));self.attn2_scale=nn.Parameter(torch.ones(dim,dtype=torch.float32));self.mlp_scale=nn.Parameter(torch.ones(dim,dtype=torch.float32))if use_mlp else None;self.resid_mix=nn.Parameter(torch.stack((torch.ones(dim),torch.zeros(dim))).float());self.ln_scale_factor=1./math.sqrt(layer_idx+1)if ln_scale else 1.;self.parallel=False
	def forward(self,x,x0,attn,attn2):
		mix=self.resid_mix.to(dtype=x.dtype);x_in=mix[0][None,None,:]*x+mix[1][None,None,:]*x0;attn_out=attn(self.attn_norm(x_in)*self.ln_scale_factor+self.attn_norm_bias.to(dtype=x_in.dtype)[None,None,:])
		if self.parallel:
			x_out=x_in+self.attn_scale.to(dtype=x_in.dtype)[None,None,:]*attn_out
			x_out=x_out+self.attn2_scale.to(dtype=x_in.dtype)[None,None,:]*attn2(self.attn2_norm(x_in)*self.ln_scale_factor+self.attn2_norm_bias.to(dtype=x_in.dtype)[None,None,:])
			if self.mlp is not None:x_out=x_out+self.mlp_scale.to(dtype=x_in.dtype)[None,None,:]*self.mlp(self.mlp_norm(x_in)*self.ln_scale_factor)
		else:
			x_out=x_in+self.attn_scale.to(dtype=x_in.dtype)[None,None,:]*attn_out
			x_out=x_out+self.attn2_scale.to(dtype=x_out.dtype)[None,None,:]*attn2(self.attn2_norm(x_out)*self.ln_scale_factor+self.attn2_norm_bias.to(dtype=x_out.dtype)[None,None,:])
			if self.mlp is not None:x_out=x_out+self.mlp_scale.to(dtype=x_out.dtype)[None,None,:]*self.mlp(self.mlp_norm(x_out)*self.ln_scale_factor)
		return x_out
class GPT(nn.Module):
	def __init__(self,h):
		super().__init__()
		...
		self.shared_attn=CausalSelfAttention(h.model_dim,h.num_heads,h.num_kv_heads,h.rope_base,h.qk_gain_init,h.train_seq_len);self.shared_attn2=CausalSelfAttention(h.model_dim,h.num_heads,h.num_kv_heads,h.rope_base,h.qk_gain_init,h.train_seq_len)
		self.num_encoder_layers=h.num_layers//2;self.num_decoder_layers=h.num_layers-self.num_encoder_layers;self.blocks=nn.ModuleList([Block(h.model_dim,h.mlp_mult,layer_idx=i,ln_scale=h.ln_scale)for i in range(h.num_layers)])
		if h.rope_dims>0:
			head_dim=h.model_dim//h.num_heads
			self.shared_attn.rope_dims=h.rope_dims;self.shared_attn.rotary=Rotary(head_dim,base=h.rope_base,train_seq_len=h.train_seq_len,rope_dims=h.rope_dims);self.shared_attn2.rope_dims=h.rope_dims;self.shared_attn2.rotary=Rotary(head_dim,base=h.rope_base,train_seq_len=h.train_seq_len,rope_dims=h.rope_dims)
		...
		if h.xsa_last_n>0:self.shared_attn.use_xsa=True;self.shared_attn2.use_xsa=True
	...
	def forward_logits(self,input_ids):
		...
		for i in enc_iter:x=self.blocks[i](x,x0,self.shared_attn,self.shared_attn2);skips.append(x)
		for(skip_idx,i)in enumerate(dec_iter):
			...
			x=self.blocks[i](x,x0,self.shared_attn,self.shared_attn2)
		x=self.final_norm(x)
...
def classify_param(name):
	...
	if'.attn.'in name or'.attn2.'in name or'shared_attn'in name or'.proj.'in name and'.mlp.'not in name:return'attn'
	return'other'
@torch.compile
...
class Muon(torch.optim.Optimizer):
	...
	def step(self,closure=None):
		...
		return loss
CONTROL_TENSOR_NAME_PATTERNS=tuple(pattern for pattern in os.environ.get('CONTROL_TENSOR_NAME_PATTERNS','attn_scale,attn_scales,attn2_scale,attn2_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,skip_weight,skip_weights,skip_gates').split(',')if pattern)
class Optimizers:
	def __init__(self,h,base_model):
		block_named_params=list(base_model.blocks.named_parameters())+[(f"shared_attn.{name}",p)for(name,p)in base_model.shared_attn.named_parameters()]+[(f"shared_attn2.{name}",p)for(name,p)in base_model.shared_attn2.named_parameters()];matrix_params=[p for(name,p)in block_named_params if p.ndim==2 and not any(pattern in name for pattern in CONTROL_TENSOR_NAME_PATTERNS)];scalar_params=[p for(name,p)in block_named_params if p.ndim<2 or any(pattern in name for pattern in CONTROL_TENSOR_NAME_PATTERNS)]
