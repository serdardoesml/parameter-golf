...
class Block(nn.Module):
	def __init__(self,dim,num_heads,num_kv_heads,mlp_mult,rope_base,qk_gain_init,train_seq_len,layer_idx=0,ln_scale=False,use_attn2=False,use_mlp=True):super().__init__();self.attn_norm=RMSNorm();self.mlp_norm=RMSNorm()if use_mlp else None;self.attn=CausalSelfAttention(dim,num_heads,num_kv_heads,rope_base,qk_gain_init,train_seq_len);self.attn2_norm=RMSNorm()if use_attn2 else None;self.attn2=CausalSelfAttention(dim,num_heads,num_kv_heads,rope_base,qk_gain_init,train_seq_len)if use_attn2 else None;self.mlp=MLP(dim,mlp_mult)if use_mlp else None;self.attn_scale=nn.Parameter(torch.ones(dim,dtype=torch.float32));self.attn2_scale=nn.Parameter(torch.ones(dim,dtype=torch.float32))if use_attn2 else None;self.mlp_scale=nn.Parameter(torch.ones(dim,dtype=torch.float32))if use_mlp else None;self.resid_mix=nn.Parameter(torch.stack((torch.ones(dim),torch.zeros(dim))).float());self.ln_scale_factor=1./math.sqrt(layer_idx+1)if ln_scale else 1.;self.parallel=False
	def forward(self,x,x0):
		...
		if self.parallel:
			...
			if self.attn2 is not None:x_out=x_out+self.attn2_scale.to(dtype=x_in.dtype)[None,None,:]*self.attn2(self.attn2_norm(x_in)*self.ln_scale_factor)
			...
			x_out=x_in+self.attn_scale.to(dtype=x_in.dtype)[None,None,:]*attn_out
			if self.attn2 is not None:x_out=x_out+self.attn2_scale.to(dtype=x_out.dtype)[None,None,:]*self.attn2(self.attn2_norm(x_out)*self.ln_scale_factor)
...
class GPT(nn.Module):
	def __init__(self,h):
		...
		self.num_encoder_layers=h.num_layers//2;self.num_decoder_layers=h.num_layers-self.num_encoder_layers;self.blocks=nn.ModuleList([Block(h.model_dim,h.num_heads,h.num_kv_heads,h.mlp_mult,h.rope_base,h.qk_gain_init,h.train_seq_len,layer_idx=i,ln_scale=h.ln_scale,use_attn2=h.loop_start<=i<=h.loop_end,use_mlp=i not in(0,7))for i in range(h.num_layers)])
		if h.rope_dims>0:
			...
			for block in self.blocks:
				...
				if block.attn2 is not None:block.attn2.rope_dims=h.rope_dims;block.attn2.rotary=Rotary(head_dim,base=h.rope_base,train_seq_len=h.train_seq_len,rope_dims=h.rope_dims)
		...
		if h.xsa_last_n>0:
			for i in range(max(0,h.num_layers-h.xsa_last_n),h.num_layers):self.blocks[i].attn.use_xsa=True;self.blocks[i].attn2 is not None and setattr(self.blocks[i].attn2,'use_xsa',True)
...
def classify_param(name):
	...
	if'.attn.'in name or'.attn2.'in name or'.proj.'in name and'.mlp.'not in name:return'attn'
	return'other'
@torch.compile
