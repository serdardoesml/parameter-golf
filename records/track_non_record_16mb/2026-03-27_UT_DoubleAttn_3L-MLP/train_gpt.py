...
# - 8 attention heads with 4 KV heads (GQA) and 16x MLP expansion
# - vocab size 1024, sequence length 1024, untied embeddings
...
class Hyperparameters:
    ...
    num_layer_schedule = {
        int(k): int(v)
        for item in os.environ.get("NUM_LAYER_SCHEDULE", "0:2,2000:6").split(",")
        if item.strip()
...
class RMSNorm(nn.Module):
    def __init__(self, dim: int | None = None, eps: float | None = None, bias: bool = False):
        super().__init__()
...
class MLP(nn.Module):
    def __init__(self, dim: int, mlp_mult: int):
        ...
        self.mid = CastedLinear(hidden, hidden, bias=False)
...
def main() -> None:
    ...
    step = 0
    while True:
        should_advance_schedule = current_schedule_idx + 1 < len(schedule_steps) and step >= schedule_steps[current_schedule_idx + 1]
        last_step = step == args.iterations or (stop_after_step is not None and step >= stop_after_step)
        should_validate = last_step or (args.val_loss_every > 0 and step % args.val_loss_every == 0)

        while should_advance_schedule and not should_validate:
            torch.cuda.synchronize()
